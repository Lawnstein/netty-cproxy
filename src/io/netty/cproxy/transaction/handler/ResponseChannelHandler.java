...
/**
 * service provider端连接处理.
 * 
 * @author Lawnstein.Chan
 * @version $Revision:$
 */
public class ResponseChannelHandler extends ChannelInboundHandlerAdapter {
	protected final static Logger logger = LoggerFactory.getLogger(ResponseChannelHandler.class);

	private ProxySession proxySession;

	public ResponseChannelHandler(ProxySession proxySession) {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg)
		...
		if (logger.isDebugEnabled())
			logger.debug(this + " Transfer from ResponseChannel "
				+ proxySession.getResponseChannel() + " to RequestChannel "
				+ proxySession.getRequestChannel() + " with message (" + msg.getClass() + ")");
//		ChannelFuture f = proxySession.getRequestChannel().writeAndFlush(msg).await();
		
		proxySession.getServiceConfig().setLastTalkTime(System.currentTimeMillis());
		ChannelFuture f = proxySession.getRequestChannel().writeAndFlush(msg);
...
//			((ByteBuf) msg).release();
	...
	public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause)
		...
		logger.info(this + " response channel exceptionCaught。");
		proxySession.destroy();
